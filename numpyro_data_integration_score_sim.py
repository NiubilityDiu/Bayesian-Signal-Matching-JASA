...
prob_0_subject_1 = 0.9
# prob_0_subject_1 = float(sys.argv[2])

group_1_mu_tar = 0.1
# group_1_mu_tar = float(sys.argv[3])
group_1_mu_ntar = group_1_mu_tar - 1.0

param_true_dict_1 = {
    'target_char': target_char,
    'N': N,
    'seq_size': [10, 10],
    'label_N': {'subject_0': np.array([1.0, 0.0]),
                'subject_1': np.array([prob_0_subject_1,
                                       1.0 - prob_0_subject_1])},
    'group_0': {'mu': np.array([[1.0], [0.0]]), 'sigma': 1.0},
    'group_1': {'mu': np.array([[group_1_mu_tar], [group_1_mu_ntar]]),
                'sigma': 1.0}
}
if prob_0_subject_1 > 0.5:
    scenario_name = 'N_{}_K_{}_selected_overlap_{}'.format(
        N, K, param_true_dict_1['group_1']['mu'][0, 0]
    )
else:
    scenario_name = 'N_{}_K_{}_not_selected_overlap_{}'.format(
        N, K, param_true_dict_1['group_1']['mu'][0, 0]
    )
# scenario_name_dir = '{}/{}'.format(
#     parent_dir, scenario_name
# )
scenario_name_dir = '{}/N_{}_K_{}/{}'.format(
    parent_dir, N, K, scenario_name
)
if not os.path.exists(scenario_name_dir):
    os.mkdir(scenario_name_dir)

sim_data_dict_1 = generate_simulated_data_score_based(
    param_true_dict_1, sim_rcp_array, scenario_name_dir
)

data_source = [sim_data_dict_1['subject_1']['X']]
data_sub0 = sim_data_dict_1['subject_0']['X']
...
for i in range(seq_size):
    ...
    nuts_kernel = NUTS(score_integration_sim_N_2)
...
# ax1[0, 0].figure(1, figsize=(10, 10))
# plot prob_1[0] w.r.t sequence size of subject 0
ax1[0].errorbar(
    np.arange(seq_size)+1, prob_1_mean, marker='o',
    yerr=prob_1_std, color='black', ecolor='gray', elinewidth=2
)
# ax1[0].set_xlabel('Sequence Number of Subject 0')
ax1[0].set_ylim([0, 1])
ax1[0].set_title(
    'prob_1[0], true value = {}'.format(
        param_true_dict_1['label_N']['subject_1'][0]
    ), fontsize=10
)
...
ax1[1].hlines(xmin=1, xmax=seq_size, y=np.array([
    param_true_dict_1['group_0']['mu'][0, 0],
    param_true_dict_1['group_0']['mu'][1, 0]]), linestyles='dashed', colors='black')
ax1[1].set_xlabel('Sequence Number of Subject 0', fontsize=10)
ax1[1].set_title(
    'mu_tar_0, mu_ntar_0, true values = {}, {}'.format(
        param_true_dict_1['group_0']['mu'][0, 0],
        param_true_dict_1['group_0']['mu'][1, 0]
    ), fontsize=10
)
...
ax1[2].hlines(xmin=1, xmax=seq_size, y=np.array([
    param_true_dict_1['group_1']['mu'][0, 0],
    param_true_dict_1['group_1']['mu'][1, 0]]), linestyles='dashed', colors='black')
# ax1[2].set_xlabel('Sequence Number of Subject 1')
ax1[2].set_title(
    'mu_tar_1, mu_ntar_1, true values = {}, {}'.format(
        param_true_dict_1['group_1']['mu'][0, 0],
        param_true_dict_1['group_1']['mu'][1, 0]
    ), fontsize=10
)
