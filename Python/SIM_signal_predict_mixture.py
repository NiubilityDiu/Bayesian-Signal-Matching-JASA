...
if local_use:
    ...
    delta_threshold = float(sys.argv[7])
...
length_ls_2 = [[0.4, 0.3] for k in range(K)]
...
for seq_i in np.arange(seq_source_size):
    ...
    signal_train_i = np.reshape(
        signal_train[:, :(seq_i+1), ...], [target_char_size * (seq_i + 1) * rcp_unit_flash_num, signal_length]
    )
    ...
    if not z_vector_delta_binary_sum:
        ...
        # testing set
        pred_score_test_0 = predict_reference_fast(
            mcmc_output_dict, signal_length, eigen_fun_mat_dict['group_0'], signal_test
        )

    else:
        print('At least one source participant is selected by threshold {}. Use BSM-Cluster for prediction.'.format(delta_threshold))
        # BSM methods here
        pred_score_train_0_i = predict_cluster_fast(
            mcmc_output_dict, signal_length, signal_train_i, mcmc_ids_seq_i
        )

        # testing set
        pred_score_test_0 = predict_cluster_fast(
            mcmc_output_dict, signal_length, signal_test, mcmc_ids_seq_i
        )
