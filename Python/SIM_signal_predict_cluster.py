...
chain_size = 3
mcmc_size_per_chain = 500
mcmc_size_total = int(mcmc_size_per_chain * chain_size)
...
scenario_name_dir = '{}/N_{}_K_{}/{}'.format(parent_dir, N, K, scenario_name)
...
scenario_name_dir = '{}/iter_{}'.format(scenario_name_dir, iter_num)
scenario_name_2_dir = '{}/borrow_gibbs'.format(scenario_name_dir, K)
...
for seq_i in np.arange(seq_source_size):
    ...
    signal_train_i = np.reshape(
        signal_train[:, :(seq_i + 1), ...], [target_num_train * (seq_i + 1) * rcp_unit_flash_num, signal_length]
    )
    code_train_i = np.reshape(
        code_train[:, :(seq_i + 1), :], [target_num_train * (seq_i + 1) * rcp_unit_flash_num]
    ).astype('int8')
    label_train_i = np.reshape(
        label_train[:, :(seq_i + 1), :], [target_num_train * (seq_i + 1) * rcp_unit_flash_num]
    )

    # import mcmc output files
    mcmc_output_dict = sio.loadmat('{}/mcmc_sub_0_seq_size_{}_cluster.mat'.format(scenario_name_2_dir, seq_i + 1))
    mcmc_ids_seq_i = None

    pred_score_train_0_i = predict_cluster_fast(mcmc_output_dict, signal_length, signal_train_i, mcmc_ids_seq_i)
    ...
    cluster_predict_train_name_dir_seq_i = '{}/{}.json'.format(scenario_name_2_dir, cluster_predict_train_name_seq_i)
    with open(cluster_predict_train_name_dir_seq_i, "w") as write_file:
        json.dump(cluster_prob_train_dict, write_file)

    # testing set
    pred_score_test_0 = predict_cluster_fast(mcmc_output_dict, signal_length, signal_test, mcmc_ids_seq_i)
    ...
    cluster_predict_test_name_dir_seq_i = '{}/{}.json'.format(scenario_name_2_dir, cluster_predict_test_name_seq_i)
