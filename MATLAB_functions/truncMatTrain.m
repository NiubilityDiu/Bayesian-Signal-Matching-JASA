function [eeg_trunc_obj] = truncMatTrain(eeg_obj, eeg_window)
...
index_tag = findStimStartOffline(eeg_tags(:, 1));
% disp(output_index);
index_begin = findStimStartOffline(eeg_begin);
% disp(output_index_2);

[flash_num, ~] = size(index_tag);
hidden_mat_size = size(eeg_tags, 2);
...
eeg_trunc_obj.Tags = eeg_trunc_tags;
